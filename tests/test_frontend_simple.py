...
def test_javascript_file_is_valid():
    ...
    project_root = Path(__file__).parent.parent
    js_file = project_root / "custom_components" / "med_expert" / "www" / "med-expert-panel.js"
    ...
    print("✓ JavaScript file contains expected content")
    print("✓ Lit framework: present")
    print("✓ Panel component: present")
    print("✓ Home Assistant integration: present")
...
def test_javascript_file_is_minified():
    ...
    assert any(len(line) > 1000 for line in lines), \
        "Should be minified with long lines"
    
    print("✓ JavaScript file is minified")
...
def test_async_setup_function_exists():
    ...
    assert "async def async_setup" in content, \
        "Should define async_setup function"
    
    # Check that it registers the static path
    assert "register_static_path" in content, \
    ...
    print("✓ async_setup function is properly defined")
    print("✓ Static path registration: present")
    print("✓ www directory reference: present")
...
def test_init_imports_pathlib():
    ...
    assert "Path(__file__)" in content, \
        "Should use Path for file operations"
...
def test_static_path_url_follows_convention():
    ...
    assert "cache_headers" in content, \
        "Should enable cache headers for static files"
    
    print("✓ Static path URL follows HA conventions")
    print("✓ Cache headers: enabled")


def test_source_map_exists():
    """Test that source map file exists for debugging."""
    project_root = Path(__file__).parent.parent
    js_map_file = project_root / "custom_components" / "med_expert" / "www" / "med-expert-panel.js.map"
    ...
    print(f"✓ Source map exists: {js_map_file}")
    print(f"✓ Source map size: {js_map_file.stat().st_size} bytes")
...
def test_typescript_source_exists():
    ...
    assert frontend_src.exists(), f"Frontend src directory should exist at {frontend_src}"
    ...
    print("✓ TypeScript source files exist")
    print(f"  - panel.ts: {panel_ts}")
    print(f"  - types.ts: {types_ts}")
...
def test_frontend_build_config_exists():
    ...
    assert webpack_config.exists(), f"webpack.config.js should exist at {webpack_config}"
    ...
    print("✓ Frontend build configuration complete")
    print("  - package.json: present")
    print("  - tsconfig.json: present")
    print("  - webpack.config.js: present")
...
def test_panel_component_structure():
    ...
    print("✓ Panel component has correct structure")
    print("  - Lit imports: present")
    print("  - Custom element: present")
    print("  - Home Assistant integration: present")
